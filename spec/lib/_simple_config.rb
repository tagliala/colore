SimpleConfig.for :application do
  ...
  load "#{Rails.root}/config/settings/application.rb",   :if_exists? => true
  ...
  load "#{Rails.root}/config/settings/#{Rails.env}.rb",  :if_exists? => true
  ...
  load "#{Rails.root}/config/settings/local.rb",         :if_exists? => true

end

C_ = SimpleConfig.for(:application)
